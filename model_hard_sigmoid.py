...
class GRU():
    ...
    def __init__(self,sequences, timesteps, inputs, outputs):
        ...
        print(self.Wz.shape)
        self.y, self.dy = np.zeros((sequences, 1), dtype = np.float16) ,np.zeros((sequences,1))
    ...
    def forward(self,X):
        ...
        for t in range(1, X.shape[1]):
            ...
            print("X", X[0:5, t, 0])
            print("R", self.r[0:5, t, 0])
        self.y = sigmoid(np.matmul(self.s[:,self.s.shape[1]-1,:],self.Wlin) + self.Blin)
        print("Y", self.y[0:5])
        return self.y
    ...
    def change_input_size(self, sequences, timesteps,outputs):
        ...
        self.y = np.zeros((sequences, timesteps, 1),dtype =np.float16)
