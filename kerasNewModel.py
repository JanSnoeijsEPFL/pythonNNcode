...
for index, (train_indices, val_indices) in enumerate(skf.split(X_new, Y_1D)):
    ...
    X_resampled = X_resampled[:,:,:,:,0].swapaxes(2,3)
    #----------------------------------------------------------------------------
    X_train = []
    ...
    for i in range(timesteps):
        x_input.append(Input(shape=(seq_number*nb_train_files, batch_size, electrodes)))
        x_gru = GRU(100, recurrent_dropout = 0.3, dropout = 0.3, return_sequences = False)(x_input)
        
    x_avg = GlobalAveragePooling1D()(x_gru)
    x_dense = Dense(x_avg, activation  = 'softmax')
    adam = Adam(lr =0.001, beta_1 = 0.9, beta_2 = 0.999, epsilon = 10.0**-7, decay = 0.0, amsgrad = False)
    model = Model([x_input[i] for i in range(timesteps)], x_dense)
    model.compile(optimizer = adam, loss = 'categorical_crossentropy', metrics = ['accuracy'])
    print(model.summary())


    model.fit([X_train[i] for i in range(timesteps)], Y_resampled, epochs = 60, shuffle = True)
